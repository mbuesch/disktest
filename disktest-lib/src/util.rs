...
pub fn prettybytes(count: u64, binary: bool, decimal: bool, bytes: bool) -> String {
    ...
    if binary {
        let _ = match count {
            ...
            PIB..EIB => write!(ret, "{:.4} PiB", ((count / GIB) as f64) / (MIB as f64)),
            TIB..PIB => write!(ret, "{:.4} TiB", ((count / MIB) as f64) / (MIB as f64)),
            GIB..TIB => write!(ret, "{:.2} GiB", ((count / MIB) as f64) / (KIB as f64)),
            MIB..GIB => write!(ret, "{:.1} MiB", (count as f64) / (MIB as f64)),
            0..MIB => write!(ret, "{:.1} kiB", (count as f64) / (KIB as f64)),
        };
    }
    ...
    if decimal {
        let _ = match count {
            ...
            PB..EB => write!(ret, "{:.4} PB", ((count / GB) as f64) / (MB as f64)),
            TB..PB => write!(ret, "{:.4} TB", ((count / MB) as f64) / (MB as f64)),
            GB..TB => write!(ret, "{:.2} GB", ((count / MB) as f64) / (KB as f64)),
            MB..GB => write!(ret, "{:.1} MB", (count as f64) / (MB as f64)),
            0..MB => write!(ret, "{:.1} kB", (count as f64) / (KB as f64)),
        };
    }
